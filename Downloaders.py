...
def download_playlist(playlist_url, download_type, output_folder, textarea, progress_bar):
    match download_type:
        case "MP4 (Alta Qualidade)":
            download_playlist_mp4(playlist_url, "HIGH", output_folder, textarea, progress_bar)
        case "MP4 (Baixa Qualidade)":
            download_playlist_mp4(playlist_url, "LOW", output_folder, textarea, progress_bar)
        case "MPG":
            download_playlist_as_mpg(playlist_url, output_folder, textarea, progress_bar)
        case _:
            textarea.insert(tk.END, "Tipo inválido...\n", "failed")

def download_playlist_mp4(playlist_url, qualityType, output_folder, textarea, progress_bar):
    global total_files_in_queue
    try:
        ...
        if video_urls == None or len(video_urls) <= 0:
            ...
            change_queue_value(len(video_urls))
        ...
        for video_url in video_urls:
            ...
            try:                
                ...
                update_progress_bar(i, total_files_in_queue, progress_bar)
    except:
        textarea.insert(tk.END, f"Ocorreu uma falha grave ao tentar baixar a playlist: {playlist_url}", "failed")
        return
    finally:
        try:
            change_queue_value(-len(video_urls))
...
def download_playlist_as_mpg(playlist_url, output_folder, textarea, progress_bar):
    global total_files_in_queue
    try:
        # Get a list of all video URLs in the playlist
        playlist = Playlist(playlist_url)
...
def change_queue_value(value):
    global total_files_in_queue

    # Adquirir o Lock antes de acessar a variável compartilhada
    lock.acquire()
    try:
        # Seção crítica do código
        total_files_in_queue += value
    finally:
        # Liberar o Lock após o acesso à variável compartilhada
        lock.release()
